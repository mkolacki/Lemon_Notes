    ...
    String evalComponent(String being_evaluated){
        ...
        if(has_internal_operations(being_evaluated)){
            if(enough_parenthesis(being_evaluated)){
                ...
                if(being_evaluated.contains("log")){
                    String temp = being_evaluated;
                    if(temp.substring(temp.indexOf("log") + 3).trim().startsWith("(")){
                        String before_the_log, log_this, dont_log_this;
                        before_the_log = temp.substring(0, temp.indexOf("log"));
                        dont_log_this = temp.substring(temp.indexOf("log") + 3).trim();
                        Integer count1, count2;
                        count1 = count2 = 0;
                        do{
                            if(dont_log_this.startsWith("(")){
                                count1++;
                            }else if(dont_log_this.startsWith(")")){
                                count2++;
                            }
                            dont_log_this = dont_log_this.substring(1);
                            /*if(dont_log_this.length() == 1){
                                if(dont_log_this.startsWith(")")){
                                    count2++;
                                }
                                dont_log_this = "";
                        ...
                        if(!dont_log_this.equals("")){
                            log_this = evalComponent(being_evaluated.substring(being_evaluated.indexOf(being_evaluated.indexOf("log") +3, being_evaluated.indexOf(dont_log_this))));

                        }else {
                            log_this = evalComponent(being_evaluated.substring(being_evaluated.indexOf("log") +3));
                            //log_this = log_this.replace(")", "");
                        }

                        while(log_this.contains("(")){
                            log_this = evalComponent(log_this);
                        }
                        //log_this = log_this.replace("(", "");
                        //log_this = log_this.replace(")", "");
                        double logged = Math.log10(Double.parseDouble(log_this));
                        return evalComponent(before_the_log + " " + Double.toString(logged) + " " + dont_log_this);
                    }
                ...
                if(being_evaluated.contains("(")){
                    ...
                    while(temp != null){
                        ...
                        if(count1 == count2){
                            String reducing, prereduced;
                            prereduced = evaluated;
                            for(int i = 0; i < count1; i++){
                                prereduced = prereduced.substring(prereduced.indexOf("(") + 1);
                            }
                            reducing = evalComponent(prereduced.substring(0, prereduced.indexOf(temp)));
                            ...
                            return evaluated = evaluated.replace(evaluated.substring(evaluated.indexOf(prereduced) - 1, being_evaluated.indexOf(temp) + 1), reducing);
                        }
